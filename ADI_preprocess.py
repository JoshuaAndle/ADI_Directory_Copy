import os
import numpy as np
#from ADI_occupancy import get_distance_map, get_grid_map


class DataProcesser:

        ### Do obs_length and pred_length change between calls, or is it always called with the number of the last obs/predicted frame for a given video?
    def __init__(self, data_dir, observed_frame_num, predicting_frame_num):
        self.data_dir = data_dir
        self.file_path = os.path.join(self.data_dir, 'pos_data.csv')
    ...
    def get_traj(self):
        ...
        for pedIndex in range(self.ped_num):
            ...
            for i in range(len(self.raw_data[1])):
                    ### I believe this 1 is just offsetting it, since the range begins at 0
    ...
    def get_traj_input(self):
        ...
        for pedIndex in range(len(self.obs)):
            person_pedIndex = []
                ### Extracts the coordinates for the given person across all indices of obs_length
            for i in range(self.obs_length):
            ...
            ### Prepares and outputs the results as a tensor of all observed trajectory data up to obs_length
...
prep = DataProcesser('./data/ETHhotel/annotation',8,12)
np.save("./data/ETHhotel/obs", prep.obs)
np.save("./data/ETHhotel/pred", prep.pred)

obs_temp = np.load("./data/ETHhotel/obs.npy")
