...
### This is just here because importing ADI_utils was being a pain due to relative import shenanigans. Replace directory as needed for now
import sys
sys.path.append('/home/josh/Desktop/ADI_Project')
...
from ADI_utils import preprocess, person_model_input, model_expected_ouput
...
def image_tensor(data_dir, frame_ID):
...
def all_image_tensor(data_dir, obs, img_width, img_height):
    ...
    for i in range(len(obs)):
        ...
        image.append(image_tensor(data_dir, int(obs[i][-1][1])))
...
# ### Run these line once to create the img.npy file to later load directly
# obs_temp = np.load("./data/ETHhotel/obs.npy")
# image_array = all_image_tensor(frame_dir_1, obs_temp, 576, 720)
# np.save("./data/ETHhotel/img_1", image_array)
...
img_1 = np.load('./data/ETHhotel/img_1.npy')
...
# img_2 = all_tensor(frame_dir_2, obs_2, 576, 720)
