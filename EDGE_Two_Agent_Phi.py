...
def Compute_MI(XW,YV, phi_type, U,eps_X,eps_Y,b_X,b_Y):
    ...
    for e in CXY.keys():
        ...
        ### Not finished, doesnt normalize in a fully sensible way
            ...
            N = 5
            dis, conv = calc_distance(XW, YV, N)
            vel, acc = calc_speed(XW, YV, N)
...
if __name__ == "__main__":
    ...
    ### Makes 2 500x2 datasets as placeholders for 500 frame trajectories    
    n_samples = 500
    np.random.seed(seed=int(time.time()))
    ...
    I = EDGE(X,Y,hashing="floor", phi_type = "dvac")
