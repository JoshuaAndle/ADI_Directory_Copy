...
### To include ADI as a filter or weighting mechanism, it should likely be best calculated in the occupancy map code when looping over each non-target agent
...
def get_rectangular_occupancy_map(frame_ID, ped_ID, dimensions, neighborhood_size, grid_size, data):
    ...
    for i in range(len(data[0])):
        if data[0][i] == frame_ID:
                ### Appends all columns for the given frame
    ...
    ### Seems to calculate the angle between a horizontal line in the +x axis and the line between the target and a give other agent
